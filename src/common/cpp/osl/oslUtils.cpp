...
//	{
//		ProjectionUtil projectionUtil;
//		listProjectionHistory(surfaceShaderNode, projectionUtil);
...
namespace MAYATO_OSLUTIL{
	...
	void OSLUtilClass::listProjectionHistory(MObject& mobject, MAYATO_OSL::ProjectionUtil& util)
	{
		MFnDependencyNode depFn(mobject);
		MStatus stat;
		MPlugArray pa;
	...
	MString OSLUtilClass::createPlugHelperNodeName(MPlug& plug, bool outType)
	{
		MPlug p = plug;
		while (p.isChild())
			p = p.parent();
	...
	MString OSLUtilClass::createPlugHelperNodeName(const char *attrName, MObject& node, bool outType)
	{
		MFnDependencyNode depFn(node);
		MPlug plug = depFn.findPlug(attrName);
		return createPlugHelperNodeName(plug, outType);
	...
	void OSLUtilClass::createPlugHelperNode(MPlug plug, bool outType)
	{
		MString helperNodeName = createPlugHelperNodeName(plug, outType);
		ATTR_TYPE at = getPlugAttrType(plug);
		MString attrName = getAttributeNameFromPlug(plug);
		...
		if (!outType)
		{
			...
			paramArray.push_back(MAYATO_OSL::OSLParameter("inX", fval0));
			paramArray.push_back(MAYATO_OSL::OSLParameter("inY", fval1));
			paramArray.push_back(MAYATO_OSL::OSLParameter("inZ", fval2));
		}
	...
	This is quite complicated, and I don't like it very much, but it somehow works at the moment....
	*/
	void OSLUtilClass::createOSLHelperNodes(ShadingNode& snode)
	{
		const char *inAttributes[] = { "inX", "inY", "inZ" };
		const char *outAttributes[] = { "outX", "outY", "outZ" };
		std::string inAttribute;
	...
	{
		MAYATO_OSL::ProjectionUtil projectionUtil;
		MPlugArray pa;
		plug.connectedTo(pa, true, false);
		if (pa.length() > 0)
		{
