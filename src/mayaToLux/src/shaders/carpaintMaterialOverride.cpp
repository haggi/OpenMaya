...
MHWRender::MPxSurfaceShadingNodeOverride* carpaintOverride::creator(const MObject& obj)
{
	return new carpaintOverride(obj);
}

carpaintOverride::carpaintOverride(const MObject& obj)
: MPxSurfaceShadingNodeOverride(obj)
{
}

carpaintOverride::~carpaintOverride()
{
}

MHWRender::DrawAPI carpaintOverride::supportedDrawAPIs() const
{
	// works in both gl and dx
	return MHWRender::kOpenGL | MHWRender::kDirectX11;
}

MString carpaintOverride::fragmentName() const
{
	// Just reuse Maya's lambert surface shader
	return "mayaLambertSurface";
}

void carpaintOverride::getCustomMappings(
...
MString carpaintOverride::primaryColorParameter() const
{
	// Use the color parameter from the lambert fragment as the primary color
	return "color";
}

MString carpaintOverride::transparencyParameter() const
{
	// Use the "transparency" parameter from the lambert fragment for transparency
	return "transparency";
}

MString carpaintOverride::bumpAttribute() const
{
