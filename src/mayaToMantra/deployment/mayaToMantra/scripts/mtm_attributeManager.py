...
mantra_node_attributes = {'mesh': ['Render As Subdiv'],
                          ...
                          'geo' : ['Raytrace Options', 'Displace Bounds']
                          }
...
class GUI(pm.ui.Window):
    ...
    def isGeo(self, obj):
        if obj.type() in ['mesh', 'nurbsSurface', 'nurbsCurve']:
            return True
        return False
    ...
    def setAttributes(self, cmd):
        ...
        if cmd == "assignSelected":
            ...
            for element in relatives:
                ...
                if self.isGeo(element):
                    print "attatch go geo"
                    for att in leafAttributes:
                        if att in mantra_node_attributes['geo']:
                            if att == 'Displace Bounds':
                                if not element.hasAttr('mtm_displacebound'):
                                    print "Adding displacementbounds to obj", element
                                    element.addAttr('mtm_displacebound', at='float')
                                    element.mtm_displacebound.set(1.0)
